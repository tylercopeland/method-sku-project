...
import { Sparkles, Paperclip, MessageSquare, FileText, Utensils, Lock, ArrowRight } from 'lucide-react';
import { Tooltip, TooltipTrigger, TooltipContent, TooltipProvider } from '@/components/ui/tooltip';

/**
 * Tooltip that explains why something is locked and the next step to unlock it.
 * Wraps any focusable/hoverable trigger.
 */
function LockTooltip({
  ...
  return (
    <Tooltip>
      ...
      <TooltipContent
        side={side}
        className="max-w-xs p-3 bg-white text-left border border-gray-200 shadow-lg"
      >
        ...
        <div className="max-w-3xl mx-auto">
          ...
          >
            ...
            <div className="flex items-center justify-between">
              ...
              {locked ? (
                <LockTooltip
                  title="Building is a Build feature"
                  body="App Studio building isn't included in your current plan. Upgrade to Build to create and edit apps with Method AI."
                  action="Upgrade to the Build plan"
                >
                  <button
                    onClick={onUpgrade}
              ...
              {aiApps.map((app) => (
                ...
                >
                  ...
                  {locked && (
                    <LockTooltip
                      side="bottom"
                      title={app.status === 'published' ? 'Saved, but read-only' : 'Saved as a draft'}
                      body={
                        app.status === 'published'
                          ? `“${app.name}” is published and safely saved, but opening and editing apps is part of Build. Your data stays intact.`
                          : `“${app.name}” is saved as a draft. Upgrade to Build to finish editing and publish it.`
                      }
                      action={app.status === 'published' ? 'Upgrade to edit this app' : 'Upgrade to finish this app'}
                    >
                      <button
                        onClick={onUpgrade}
